...
import time
# have to ask markus whether we should normalize the adv grads, probably not neededi


# privileged is male, unprivileged is female
def train_pipeline(data, sens_feature, backend, debiased=True, multiple=False):
    # load data
    ...
    clf = AdversarialDebiasing([sens_feature], adversary_loss_weight=0.7, num_epochs=n_epoch, batch_size=256,
                               random_state=279, debias=debiased, multiple=multiple)
    start = time.time()
